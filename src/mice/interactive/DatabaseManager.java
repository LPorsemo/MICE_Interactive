...
public class DatabaseManager {
    ...
    public String konverteraTillSid(String spelNamn) {

        String sid = "";

        String query = "SELECT SID FROM SPELPROJEKT WHERE BETECKNING = '" + spelNamn + "'";
        try {
            sid = database.fetchSingle(query);
        } catch (InfException ex) {
            System.out.println("BLÄV FÄL!");
        }
    ...
    public String konverteraTillKid(String domanNamn) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "FELEFELFEL");
        }
    ...
    public String setDomanBeskrivning(String kid) {
        String beskrivning = "";
        String query = "SELECT BESKRIVNING FROM KOMPETENSDOMAN WHERE KID = '" + kid + "';";

        try {
            beskrivning = database.fetchSingle(query);
        } catch (InfException ex) {
            Logger.getLogger(DatabaseManager.class.getName()).log(Level.SEVERE, null, ex);
        }
        return beskrivning;
    ...
    public ArrayList<String> getGammlaSpelprojektLista() {
        ArrayList<String> projektnamn = new ArrayList<>();
        
        
        Date dag = new Date();
        SimpleDateFormat form = new SimpleDateFormat("yyyy-MM.dd");
        String Idag = form.format(dag);
        String query = "SELECT BETECKNING FROM SPELPROJEKT WHERE Startdatum < '" + Idag + "'"
        + "AND RELEASEDATUM <= '" + Idag + "';";
        try {
        ...
        public ArrayList<String> getPagaendeSpelprojektLista() {
        ArrayList<String> projektnamn = new ArrayList<>();
        ...
        public ArrayList<String> getKommandeSpelprojektLista() {
        ArrayList<String> projektnamn = new ArrayList<>();
        ...
        public void setNyttProjektNamn(String Namn, String sid)
        {
            String query = "UPDATE SPELPROJEKT SET BETECKNING = '" + Namn + "' WHERE "
                   + "SID = '" + sid + "';" ;
        try {
            database.update(query);
        } catch (InfException ex) {
